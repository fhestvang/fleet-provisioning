...
scw-instance-verify hostname:
    ...
    ssh -o BatchMode=yes -o ConnectTimeout=8 -o StrictHostKeyChecking=accept-new "fhestvang@$tailscale_ip" 'set -euo pipefail; export PATH="$HOME/.local/bin:$HOME/.local/share/mise/shims:$PATH"; export BAO_ADDR="${BAO_ADDR:-https://bao.olm-hops.ts.net}"; export VAULT_ADDR="$BAO_ADDR"; hostname; whoami; chezmoi data | grep -E "hostname|role|hasFhhToolkit"; for c in docker mise node nvim lazygit starship fzf zoxide fd rg bat eza atuin direnv yazi tmux sesh btop gh gh-dash kubectl k9s k3d dagger skaffold devpod glow lazydocker codex claude pi; do printf "%s=" "$c"; command -v "$c"; done; groups | grep -w docker; docker info >/dev/null; bao kv get -field=GITHUB_TOKEN kv/projects/fos/shared/github-cli >/dev/null; test -d ~/github/fhh-toolkit/.git; crontab -l | grep chezmoi-sync'
